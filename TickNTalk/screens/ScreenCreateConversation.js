...
export default ScreenCreateConversation = () => {
  ...
  useEffect(() => {
    if (user && rawUsers) {
      setListUsers(Object.values(rawUsers).filter(u => (u.email !== user.email) && checkEnoughUserInfo(u).isValid));
    }
  }, [user, rawUsers]);

  const handleCreateConversationPress = () => {
    const listMembers = Object.keys(selectedUsers).filter(uEmail => selectedUsers[uEmail]);
    listMembers.push(user.email);

    if (listMembers.length <= 1) {
      showMessage({ type: "danger", message: "Please select at least 1 member to join your conversation." })
      return;
    }
  ...
  const handleToggleSelectedUser = (email) => {
    setSelectedUsers(prev => ({ ...prev, [email]: prev[email] ? false : true }));
  }
      ...
      <SafeAreaView style={SafeView}>
        <Layout style={{ flex: 1 }}>
          <Layout style={[styles.center]}>
            ...
            <ScrollView>
              {listUsers.map(user =>
                <TouchableOpacity onPress={() => handleToggleSelectedUser(user.email)}>
                  <MessageCard
